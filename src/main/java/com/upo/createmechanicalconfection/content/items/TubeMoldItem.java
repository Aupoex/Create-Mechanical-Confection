...
public class TubeMoldItem extends Item {

    public TubeMoldItem(Properties properties) {
        super(properties);
    }
    @Override
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (clickedState.is(CMCBlocks.RAW_CAKE_BATTER_BLOCK.get())) {
            if (!level.isClientSide) {
                level.setBlock(pos, CMCBlocks.TUBE_CAKE_BATTER_BLOCK.get().defaultBlockState(), 3);
